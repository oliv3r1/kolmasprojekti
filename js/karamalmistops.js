const query = `
{
  stopsByRadius(lat: 60.223925, lon: 24.758075, radius: 500) {
    edges {
      node {
        distance
        stop {
...
fetch('https://api.digitransit.fi/routing/v1/routers/hsl/index/graphql', {
    ...
    .then(data => {
        ...
        // Initialize kartta
        const map = L.map('mapid', {zoomControl: false}).setView([60.223925, 24.758075], 16);
        ...
        const metropoliaMarker = L.marker([60.223925, 24.758075], {icon: L.icon({
                iconUrl: 'https://raw.githubusercontent.com/pointhi/leaflet-color-markers/master/img/marker-icon-red.png',
                iconSize: [25, 41],
                iconAnchor: [12, 41],
                popupAnchor: [0, -41]
            })}).addTo(map)
            .bindPopup('Metropolia Karamalmi Kampus')
            .openPopup();
